...
describes what users get. Pick a format with `-o`, or let ch choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ch <command> -o table   # aligned columns for reading
ch <command> -o jsonl   # one JSON object per line, for piping
ch <command> -o json    # a single JSON array
ch <command> -o csv     # spreadsheet friendly
ch <command> -o tsv     # tab-separated
ch <command> -o url     # just the URL column
ch <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ch <command> --fields id,title,url
```
...
```bash
ch <command>            # a table, because this is a terminal
ch <command> | wc -l    # JSONL, because this is a pipe
```
