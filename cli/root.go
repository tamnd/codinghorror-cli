// Package cli builds the ch command tree on top of the codinghorror library.
package cli
...
type App struct {
	client *codinghorror.Client
	cfg    codinghorror.Config
...
func Root() *cobra.Command {
	app := &App{cfg: codinghorror.DefaultConfig()}

	root := &cobra.Command{
		Use:   "ch",
		Short: "Browse Coding Horror blog by Jeff Atwood",
		Long: `ch reads Jeff Atwood's Coding Horror blog through its public RSS feed.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

ch is an independent tool and is not affiliated with Jeff Atwood or Coding Horror.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = codinghorror.NewClient(a.cfg)
	return nil
}
